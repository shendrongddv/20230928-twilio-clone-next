  ...
  return (
    <footer className="bg-[#121C2D] pb-8 pt-16">
      <div className="container flex flex-col items-center gap-16">
        {/* # */}
        <h2 className="text-center text-2xl font-light capitalize italic text-[#8891aa] md:text-[40px]">
        ...
        <div className="flex w-full flex-col items-center gap-2 text-center">
          <ul className="mb-4 flex flex-wrap items-center justify-center gap-2 gap-x-4">
            {PrivacyNav?.map((item) => (
              <li key={item.id}>
                <Link
                  href={item.url}
                  aria-label={item.label}
                  className="text-xs text-[#e1e3ea] hover:text-[#8891aa]"
                >
                  {item.label}
                </Link>
              </li>
